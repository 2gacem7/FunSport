    ...
    <div class="card m-3 bg-light" style="max-height: 50rem; max-width: 40rem">
        ...
        <div class="card-body m-0 p-0 overflow-auto text-dark" v-if="infos[0]">
            ...
            <table class="table">
                ...
                <tbody>
                    <tr v-for="player in infos[0].players" :key="player.player_key" class="w-100">
                        <td scope="col">
                            <p class="font-weight-bold">{{player.player_name}}</p>
                            <p>{{player.player_age}} years old</p>
                            <p><a class="font-weight-bold">Country:</a>{{player.player_country}}</p>
                        </td>
                        <td scope="col" class="text-center row">
                            <p><img src="../assets/soccer-field.png" style="width: 2rem; height:2rem"
                                    class="mr-1" alt="no image match played" />{{player.player_match_played}}
                                <img src="../assets/ballon.png" style="width: 1.5rem; height:1.5rem"
                                    class="mr-1" alt="no image goals" />{{player.player_goals}}</p>

                            <p> <img src="../assets/yellow-card.png"
                                    style="width: 1.5rem; height:1.5rem" alt="no image yellow cards" />{{player.player_yellow_cards}}
                                <img src="../assets/red-card.png" style="width: 1.5rem; height:1.5rem"
                                    class="ml-2" alt="no image red cards" />{{player.player_red_cards}}</p>
                        </td>
...
<script>
    ...
    export default {
        ...
        async mounted() {
            ...
            await fetch("https://apiv2.apifootball.com/?action=get_teams&league_id=" + this.id_tournament +
                    "&team_id= " + this.team_id + "&APIkey=" + ENV.API_FOOTBALL,
                    requestOptions)
