<template>
  <div class="card m-3 bg-light" style="max-height: 50rem; max-width: 40rem">
    <div class="card-header">
      <h3 v-if="infos[0]" class="text text-center">
        {{ infos[0].team_name }}'s informations
      </h3>
    </div>
    <div class="card-body m-0 p-0 overflow-auto text-dark" v-if="infos[0]">
      <img v-if="infos[0]" class="row mx-auto m-3" :src="return_Link(infos[0].team_badge)" alt="no team badge" style="max-width: 7rem" />
        ...
        <tbody>
          <tr v-for="player in infos[0].players" :key="player.player_key" class="w-100">
            <td scope="col">
              ...
              <p>
                <a class="font-weight-bold">Country:</a>{{ player.player_country }}
              </p>
            </td>
            <td scope="col" class="text-center row">
              <p>
                <img src="../assets/soccer-field.png" style="width: 2rem; height: 2rem" class="mr-1"
                  alt="no image match played" />{{ player.player_match_played }}
                <img src="../assets/ballon.png" style="width: 1.5rem; height: 1.5rem" class="mr-1"
                  alt="no image goals" />{{ player.player_goals }}
              </p>

              <p>
                <img src="../assets/yellow-card.png" style="width: 1.5rem; height: 1.5rem"
                  alt="no image yellow cards" />{{ player.player_yellow_cards }}
                <img src="../assets/red-card.png" style="width: 1.5rem; height: 1.5rem" class="ml-2"
                  alt="no image red cards" />{{ player.player_red_cards }}
              </p>
            </td>
