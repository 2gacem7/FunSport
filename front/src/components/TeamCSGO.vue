<template>
    <div class="card m-3 bg-light" style="max-height: 30rem; max-width: 30rem">
        <div class="card-header d-flex justify-content-between">
            <h3 class="text-dark text-center m-3"> CSGO TEAM :  {{ name }} </h3>
        ...
        <button class="btn btn-danger p-1" style="width:100%" @click="remove()">Unfav</button>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        async mounted() {
            ...
            let infos = [];
            await fetch("https://api.pandascore.co/csgo/teams?sort=name&per_page=100&search%5Bslug%5D="+ this.name, {
                    "method": "GET",
                    "headers": {
