  ...
  methods: {
    ...
    * api uses: pandascore
    ...
    async getMatches() {
      ...
      await fetch(
        `/pandascore/${this.apiName}/matches?search[name]=${this.searchValue}&sort=-scheduled_at`,
        requestOptions
      )
    ...
    async getCompetitions() {
      ...
      this.results = await fetch(
        `/pandascore/${this.apiName}/tournaments?search[slug]=${this.searchValue}&sort=-begin_at`,
        requestOptions
      )
        .then((response) => response.json())
    },
