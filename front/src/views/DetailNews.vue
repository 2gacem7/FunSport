            ...
            <div v-if="commentary.commentary != ''">
              <div class="card-header d-flex">
                <span
                  >Post the
                  {{
                    parseFloat(commentary.createdAt)
                      | moment("MMMM Do YYYY, h:mm")
                  }}
                  by {{ commentary.userId }}</span
                >
                <button
                  v-if="
                    !commentary.isReported && $store.state.UserData.id != ''
                  "
                  class="btn btn-primary mr-2 ml-auto report"
...
export default {
  ...
  methods: {
    /**
     * This method is used to get data for a specific news
     * @public
     */
    async fetchData() {
      ...
      await fetch(`http://${ENV.API_BACKEND}/news/${this.newsId}`, options)
